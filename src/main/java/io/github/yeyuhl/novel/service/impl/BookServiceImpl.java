...
import io.github.yeyuhl.novel.core.common.req.PageReqDto;
...
import io.github.yeyuhl.novel.dao.entity.*;
import io.github.yeyuhl.novel.dao.mapper.BookChapterMapper;
import io.github.yeyuhl.novel.dao.mapper.BookCommentMapper;
...
import io.github.yeyuhl.novel.dto.req.BookAddReqDto;
import io.github.yeyuhl.novel.dto.req.ChapterAddReqDto;
import io.github.yeyuhl.novel.dto.req.ChapterUpdateReqDto;
import io.github.yeyuhl.novel.dto.req.UserCommentReqDto;
import io.github.yeyuhl.novel.dto.resp.*;
import io.github.yeyuhl.novel.manager.cache.*;
import io.github.yeyuhl.novel.manager.dao.UserDaoManager;
import io.github.yeyuhl.novel.manager.mq.AmqpMsgManager;
import io.github.yeyuhl.novel.service.BookService;
...
public class BookServiceImpl implements BookService {
    ...
    public RestResp<List<BookInfoRespDto>> listRecBooks(Long bookId) throws NoSuchAlgorithmException {
        ...
        int count = 0;
        // 使用getInstanceStrong()创建一个安全的随机数生成器，注意的是该方法在linux中可能存在性能问题
        Random rand = SecureRandom.getInstanceStrong();
        // 当推荐的书籍数量小于4本时
        while (count < REC_BOOK_COUNT) {
            // 产生随机索引
            int recIdIndex = rand.nextInt(lastUpdateIdList.size());
            // 如果该索引不在recIdIndexList中，则将其添加到其中，并将该索引对面的小说的信息存到respDtoList中
    ...
    public RestResp<Long> getPreChapterId(Long chapterId) {
        ...
        // 在小说章节表中查询与给定小说ID匹配的小说
        // 然后查询章节号小于给定章节号的记录，并按章节号降序排序，返回第一条记录。
    ...
    public RestResp<Long> getNextChapterId(Long chapterId) {
        ...
        QueryWrapper<BookChapter> queryWrapper = new QueryWrapper<>();
        // 在小说章节表中查询与给定小说ID匹配的小说
        // 然后查询章节号大于给定章节号的记录，并按章节号升序排序，返回第一条记录。
        queryWrapper.eq(DatabaseConsts.BookChapterTable.COLUMN_BOOK_ID, bookId)
    ...
    public RestResp<BookCommentRespDto> listNewestComments(Long bookId) {
        ...
        if (commentTotal > 0) {
            ...
            // 查询评论用户信息，并设置需要返回的评论的用户名，其中Function.identity()是静态方法
            // Function.identity()函数总是返回其输入参数，因此这里表示Map的值就是用户信息对象本身，比如
    ...
    public RestResp<PageRespDto<UserCommentRespDto>> listComments(Long userId, PageReqDto pageReqDto) {
        ...
        return RestResp.ok(PageRespDto.of(pageReqDto.getPageNum(), pageReqDto.getPageSize(), page.getTotal(), Collections.emptyList()));
    }
